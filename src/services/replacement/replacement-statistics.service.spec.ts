...
describe('convertToReplacementStatistics', () => {
    ...
    it('should aggregate replacements by category and count occurrences', () => {
        ...
        const speakersStats = result.find(s => s.category === 'Speakers');
        ...
        const aToB = speakersStats?.replacements.find(r => r.from === 'A' && r.to === 'B');
        expect(aToB?.count).toBe(3);  // A→B apparaît 3 fois

        const aToC = speakersStats?.replacements.find(r => r.from === 'A' && r.to === 'C');
        expect(aToC?.count).toBe(1);  // A→C apparaît 1 fois

        const dToE = speakersStats?.replacements.find(r => r.from === 'D' && r.to === 'E');
        expect(dToE?.count).toBe(1);  // D→E apparaît 1 fois

        // Vérifie la catégorie 'Other'
        const otherStats = result.find(s => s.category === 'Other');
