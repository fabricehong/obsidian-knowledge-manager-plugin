...
export async function createChatAgentExecutor({
  ...
  processingLLM: any;
}): Promise<RunnableWithMessageHistory<{ input: string }, any>> {
  // ---------------------------------------------------------------------------
  // TOOL (injecté, voir tools/search-vault.tool.ts)
  // ---------------------------------------------------------------------------
  // Tool de décomposition de question complexe
  const { tool: decomposeQueryTool } = createDecomposeQueryTool({ processingLLM });

  // Tool intelligent de recherche sémantique
  const { tool: intelligentSearchTool } = createSearchVaultTool({
    chatSemanticSearchService,
    processingLLM
  });

  // Ordre des tools conforme au prompt (1. decompose_query, 2. search_vault)
  ...
  // ---------------------------------------------------------------------------
  // Mémoire conversationnelle : BufferWindowMemory (15 messages)
  // ---------------------------------------------------------------------------
  // const memory = new BufferWindowMemory({ k: 15, memoryKey: AGENT_MEMORY_KEY });
